...
def test_nb_no_norm(in_data):
    conn = in_data[0]
    df_train = in_data[1]
    df_test = in_data[2]
    
    triple = conn.sql("SELECT list(agg), list(target) FROM (SELECT sum_to_nb_agg_4_0(s_length, s_width, p_length, p_width) as agg, target from iris_train group by target);").fetchall()
    
    str_triple = str(triple[0][0])
    str_labels = str(triple[0][1])
    cast = "::STRUCT(N int, lin_agg FLOAT[], quad_agg FLOAT[], lin_cat STRUCT(key INT, value FLOAT)[])[]"
    query = "select nb_train("+str_triple+cast+", "+str_labels+"::int[])"
    params = conn.sql(query).fetchall()
    predict = conn.sql("SELECT id, nb_predict("+str(params[0][0])+"::float[], false, s_length, s_width, p_length, p_width) as pred from iris_test").df()
    ...
    print("Accuracy SKLearn Gaussian NB ", acc_sklearn)
